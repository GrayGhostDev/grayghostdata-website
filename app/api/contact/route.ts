...
export async function POST(req: Request) {
  try {
    ...
    // Lazy-init the mail client so `next build` does not require the key at build time
    ...
    const { data, error } = await resend.emails.send({
      from: "Gray Ghost Data <contact@grayghostdata.com>",
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  <div style="font-size: 12px; color: #64748b; margin-top: 20px;">
    <p>This email was sent from the contact form on grayghostdata.com</p>
  </div>
</div>
      `,
      replyTo: email,
